...
public class PermissionExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<Object> handleJobException(WebRequest webRequest, PermissionException permissionException) {
        ProblemDetail problemDetail = ProblemDetail.forStatusAndDetail(permissionException.getStatus(), permissionException.getMessage());
        problemDetail.setInstance(URI.create(webRequest.getDescription(false)));
        problemDetail.setTitle(permissionException.getClass().getSimpleName());
        problemDetail.setStatus(permissionException.getStatus().value());
        problemDetail.setProperty(TIMESTAMP, LocalDateTime.now());
        return new ResponseEntity<>(problemDetail, permissionException.getStatus());
    }
